...
public class UserController:Controller
{
    ...
    public UserController(ICoreServiceManager coreServiceManager, IMapper mapper, ILogger<UserController> logger, IUnitOfWork work, IMessageService messageService)
    {
    ...
    public async Task<IActionResult> ForgetPassword(ForgetPasswordViewModel? model)
    {
        if (ModelState.IsValid)
        {
            ...
            {
                //BackgroundJob.Enqueue(() => MessageService.SendMessageAsync(user.Phonenumber, "DigiRose", code));
                return RedirectToAction("VerifyUser", "User",new{Phone = model.Phonenumber});
            }
        }
        return View(model);
    }
